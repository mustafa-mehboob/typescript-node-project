...
let getRandomNumber = (minimum: number, maximum: number) => {
  let temp = 0;
...
async function GAME() {
  ...
  const menueQuestion: QuestionInterface = {
    name: "menueInput",
    type: "list",
    choices: ["Play", "Quit"],
  };
  const battleQuestion: QuestionInterface = {
    name: "battleInput",
    type: "list",
  ...
  const enemyMaxAttack: number = 25;
  let enemyAttack: number;
  const enemyMaxHealth: number = 75;
      ...
      while (enemyHealth > 0 && playerHealth > 0) {
        do {
          battleResponse = String(
            (await inquirer.prompt(Questions[2])).battleInput
          );
          firstRun = false;
